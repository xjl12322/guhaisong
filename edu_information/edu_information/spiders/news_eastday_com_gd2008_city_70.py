...
class XueqianSpider(scrapy.Spider):
    name = "news_eastday_com_gd2008_city_70"
    # allowed_domains = ["news.eastday.com"]
    start_urls = ["http://news.eastday.com/gd2008/city/index.html"]
    ...
    def parse_detail(self,response):
        ...
        #标题title
        title =  response.xpath('//p[@class="title"]/text()').extract_first(default="")
        title = title.strip()
        ...
        if title:
            ...
            try:
                ...
                summary = summary.replace("东方网-区域频道-", "")
            except Exception as e:
                summary = ""
            summary = summay_slice(summary)
            index_node = response.xpath('string(//p[@class="info"])').extract_first()
...
# '来源:新华社 作者:胡浩 林晖 朱基钗 史竞男 选稿:刘晓晶 '
            ...
            try:
                ...
                try:
                    writer = re.search(r".*?来源：.*?作者：(.*)", index_node, re.S).group(1)
                    writer = writer.strip()
                except Exception as e:
                    print(e,"writer")
            ...
            content = response.xpath('//div[@class="nr"]').extract_first()
