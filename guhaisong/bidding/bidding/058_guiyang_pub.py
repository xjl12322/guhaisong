...
class GovBuy(object):
    ...
    def __init__(self):
        ...
        self.rq = Rdis_Queue(host='localhost', dblist='guiyang_gcjs_gyggzy_cn_list1', dbset='guiyang_gcjs_gyggzy_cn_set1')
    ...
    def load_get(self,categoryId, types, page):
        try:
            url = 'http://www.gcjs.gyggzy.cn/{}/index_{}.htm'.format(types,page)
            response = requests.get(url=url, headers=self.headers).content.decode('gb18030')
            selector = etree.HTML(response)
        except Exception as e:
    ...
    def run(self):
        ...
        task_li = [
                {'categoryId':'', 'types':'noticeconstruct','all_page': 2},
                {'categoryId':'', 'types':'noticeservice','all_page': 1},
                {'categoryId':'', 'types':'noticedesign','all_page': 1},
                {'categoryId':'', 'types':'noticereconnaissance','all_page': 1},
                {'categoryId':'', 'types':'noticequipment','all_page': 1},
                {'categoryId':'', 'types':'noticeContracting','all_page': 1},
                {'categoryId':'', 'types':'succonstruct','all_page': 2},
                {'categoryId':'', 'types':'succservice','all_page': 1},
                {'categoryId':'', 'types':'succdesign','all_page': 1},
                {'categoryId':'', 'types':'succreconnaissance','all_page': 1},
                {'categoryId':'', 'types':'succequipment','all_page': 1},
                {'categoryId':'', 'types':'contracting','all_page':1},
                {'categoryId':'', 'types':'buildNewsConstruts','all_page':2},
                {'categoryId':'', 'types':'buindNewsService','all_page':1},
                {'categoryId':'', 'types':'buildNewsDesigner','all_page':1},
                {'categoryId':'', 'types':'buildNewsrecon','all_page':1},
                {'categoryId':'', 'types':'buildNewsEuqip','all_page':1},
                {'categoryId':'', 'types':'buildContracting','all_page':1},
                {'categoryId':'', 'types':'directPub','all_page':1},
            ]
        count = 2
        for task in task_li:
