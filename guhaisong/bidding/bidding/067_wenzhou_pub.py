...
class GovBuy(object):
    ...
    def __init__(self):
        ...
        self.headers = {
            ...
            'Referer': 'http://ggzy.wzzbtb.com:6081/wzcms/jyxx/index.htm',
        ...
        self.rq = Rdis_Queue(host='localhost', dblist='wenzhou_ggzy_wzzbtb_com_list1', dbset='wenzhou_ggzy_wzzbtb_com_set1')
    ...
    def load_get_html(self, url):
        ...
        try:
            ...
            source = 'http://ggzy.wzzbtb.com:6081/'

            table_ele  = selector.xpath('//div[@class="Content-Main FloatL"]')
            if table_ele != []:
            ...
            retult_dict['zh_name'] = '温州市公共资源交易网'
            retult_dict['en_name'] = 'Wenzhou Public resource'
            # print(retult_dict)
    ...
    def load_get(self,categoryId, types, page):
        try:
            url = 'http://ggzy.wzzbtb.com:6081/wzcms/jyxx/index_{}.htm'.format(types,page)
            response = requests.get(url=url, headers=self.headers).content.decode('utf-8')
            selector = etree.HTML(response)
        except Exception as e:
    ...
    def run(self):
        ...
        task_li = [
                {'categoryId':'', 'types':'jyxx','all_page': 2},
                {'categoryId':'', 'types':'zfcgcggg','all_page': 2},
                {'categoryId':'', 'types':'zfcgzbgg','all_page': 1},
            ]
        count = 1
        for task in task_li:
