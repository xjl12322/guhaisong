...
class GovBuy (object):
    ...
    def __init__(self):
        ...
        self.headers = {
            ...
            'Referer': 'http://www.nxzfcg.gov.cn/ningxiaweb/002/002001/7.html',
        ...
        self.rq = Rdis_Queue (host='localhost', dblist='ningxia_nxzfcg_gov_cn_list1',
                              dbset='ningxia_nxzfcg_gov_cn_set1')
    ...
    def load_get_html(self, url):
        ...
        try:
            ...
            title = selector.xpath ('//h3[@class="ewb-main-h2"]/text()')
            if title != []:
                title = re.sub (r'\r|\n|\s', '', ''.join (title))
                try:
            ...
            publish_date = selector.xpath ('//div[@class="ewb-main-bar"]//text()')
            # print(publish_date)
            if publish_date != []:
            ...
            content_html = etree.tostring (table_ele, encoding="utf-8", pretty_print=True, method="html").decode (
                'utf-8')
            ...
            retult_dict['zh_name'] = '宁夏公共资源交易网'
            retult_dict['en_name'] = 'Ningxia Public resource'
    ...
    def load_get(self, categoryId, types, page):
        try:
            # url = 'http://www.nxggzyjy.org/ningxiaweb/002/7.html'
            url = 'http://www.nxggzyjy.org/ningxiaweb/{}/{}.html'.format (types, page)
            response = requests.get (url=url, headers=self.headers).content.decode ('utf-8')
            selector = etree.HTML (response)
        except Exception as e:
    ...
    def run(self):
        ...
        task_li = [
            {'categoryId': '', 'types': '002', 'all_page': 3},
        ]
        count = 2
        for task in task_li:
