...
class XueqianSpider(scrapy.Spider):
    name = "news_eastday_com_eastday_13news_auto_news_sports_67"
    # allowed_domains = ["news.eastday.com"]
    start_urls = ["http://news.eastday.com/eastday/13news/auto/news/sports/index_K48.html"]
            ...
            if url == None or url =="":
                ...
                if BL:
                    if self.bf.isContains(url):  # 判断字符串是否存在
                        ...
                        self.bf.insert(url)
                        print("请求详情页：",url)
                        yield scrapy.Request(url,callback=self.parse_detail,meta={"time_node":time_node})
                else:
                    yield scrapy.Request(url, callback=self.parse_detail, meta={"time_node": time_node})
    ...
    def parse_detail(self,response):
        ...
        #标题title
        title =  response.xpath('//h1[@class="tit"]/text()').extract_first(default="")
        title = title.strip()
...
# '来源:新华社 作者:胡浩 林晖 朱基钗 史竞男 选稿:刘晓晶 '
            ...
            try:
                ...
                index_node = response.xpath('string(//div[@class="time"]/p/span)').extract_first(default="")
                if int(len(index_node)) <= 3:
                    writer = index_node
                    writer = writer.strip()
            ...
            content = response.xpath('//div[@class="txt mt15"]').extract_first()
