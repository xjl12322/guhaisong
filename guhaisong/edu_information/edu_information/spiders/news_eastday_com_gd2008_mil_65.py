...
class XueqianSpider(scrapy.Spider):
    name = "news_eastday_com_gd2008_mil_65"
    # allowed_domains = ["news.eastday.com"]
    start_urls = ["http://news.eastday.com/gd2008/mil/index.html"]
    ...
    def parse_detail(self,response):
        ...
        #关键字keyman
        keyman = response.xpath('''//title/text()''').extract_first(default="")
        if keyman:
        ...
        if title:
            ...
            index_node = response.xpath('string(//div[@class="gray20 textcenter time"])').extract_first()

            try:
                time_node = re.search(r".*?(\d{4}-\d{1,2}-\d{1,2} \d{1,2}:\d{1,2}:\d{1,2}).*?", index_node).group(1)
...
# '来源:新华社 作者:胡浩 林晖 朱基钗 史竞男 选稿:刘晓晶 '
            ...
            #新闻内容content
            content = response.xpath('//div[@id="gjzw"]').extract_first()
