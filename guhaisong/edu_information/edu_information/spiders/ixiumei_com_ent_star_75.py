...
class XueqianSpider(scrapy.Spider):
    name = "ixiumei_com_ent_star_75"
    # allowed_domains = ["eol.cn"]
    start_urls = ["https://www.ixiumei.com/ent/star/1.shtml","https://www.ixiumei.com/ent/film/1.shtml","https://www.ixiumei.com/ent/music/1.shtml"]
    ...
    def parse(self, response):
        ...
        node_obj = response.xpath('''//ul[contains(@class,"list-three")]/li''')
        if not node_obj:
            print("error_spider",self.name)
        for detail in node_obj:
            url = detail.xpath('h2/a/@href').extract_first()
            ...
            if url == None or url =="":
                ...
                if BL:
                    if self.bf.isContains(url):  # 判断字符串是否存在
                        ...
                        self.bf.insert(url)
                        print("请求详情页：",url)
                        yield scrapy.Request(url,callback=self.parse_detail,meta={"titlepic_image":titlepic_image,"new_time":new_time})
                else:
    ...
    def parse_detail(self,response):
        ...
        if title:
            ...
            index_node = response.xpath('''string(//p[@class="news-titbar"])''').extract_first()
            try:
                time_node = response.meta.get("new_time","")
            ...
            try:
                source = re.search(r".*?来源：(.*?)\d{4}-\d{1,2}-\d{1,2} \d{1,2}:\d{1,2}:\d{1,2}.*?", index_node).group(1)
                source = source.strip()
            except Exception as e:
                print(e, "source")
